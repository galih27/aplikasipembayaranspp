...
$pageQry = $koneksidb->query($pageSql) or die("Error paging: " . $koneksidb->error);
$jumData = $pageQry->num_rows;
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td width="47%" align="right">
                    <form class="form-inline" method="post" style="margin-top:0px; " action="?page=bayar_tampil" name="cari">
...
$mySql = "SELECT pembayaran.*, pembayaran_item.uang_bayar, biaya, jns_bayar FROM pembayaran
    LEFT JOIN pembayaran_item ON pembayaran.kd_pembayaran=pembayaran_item.kd_pembayaran 
    ORDER BY pembayaran.tgl_bayar ASC LIMIT $mulai, $barisData ";
$myQry = $koneksidb->query($mySql) or die("Gagal Query Tmp" . $koneksidb->error);
$jumData = $myQry->num_rows;
$nomor = $mulai;
while ($myData = $myQry->fetch_assoc()) { // Use fetch_assoc() to get an associative array
    $nomor++;
