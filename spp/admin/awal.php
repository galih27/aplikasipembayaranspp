...
TU SMK KORPRI Sumedang merupakan aplikasi yang digunakan untuk membantu petugas TU dalam Transaksi Pembayaran </div>
	...
	$data = mysqli_query($koneksidb, $sql); // Use mysqli_query instead of mysql_query
	$jumlah_anggota = mysqli_num_rows($data); // Use mysqli_num_rows instead of mysql_num_rows

	$simpanan = mysqli_query($koneksidb, "SELECT pembayaran.*, pembayaran_item.uang_bayar, biaya, jns_bayar FROM pembayaran
              LEFT JOIN pembayaran_item ON pembayaran.kd_pembayaran = pembayaran_item.kd_pembayaran");
	$jumlah_simpanan = mysqli_num_rows($simpanan); // Use mysqli_num_rows instead of mysql_num_rows
	?>
