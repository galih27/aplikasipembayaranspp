...
$bariss = 90000;
$halaman = isset($_GET['hal']) ? (int)$_GET['hal'] : 0;
$mulai = $halaman * $bariss;
$pageSql = "SELECT * FROM siswa ORDER BY no_induk ASC LIMIT $mulai, $bariss";
$sql1 = mysqli_query($koneksidb, $pageSql);
...
<h2>MANAJEMEN DATA SISWA KELAS X<br /></h2>
<div class="main-content">
            ...
            <tr>
                <td width="53%"> <a href="?page=addsiswa1">
                        <button class="btn btn-primary">
                            <i class="fa fa-plus"></i>Tambah Siswa Kelas X
                        </button>
                    </a> <a href="?page=data_siswa12">
                        <button class="btn btn-warning">
                            <i class="fa fa-refresh"></i> Lihat Semua Data
                        </button>
                    </a>
                    <a href="simpansiswaxls.php">
                        <button class="btn btn-info">
                            <i class="fa fa-save"></i> Simpan Excel
                        </button>
                    </a> </td>
                <td width="47%" align="right">
                    <form class="form-inline" method="post" style="margin-top:0px;" action="?page=data_siswa12">
                        <?php
                        $mysqli = new mysqli("localhost", "root", "", "tu71");
                        $result = $mysqli->query("SELECT DISTINCT kelas FROM siswa");
                        ...
                        echo '</select>';
                        ?>
                        <input class="input-xlarge form-control" value="<?php echo $q; ?>" placeholder="Cari Jenis Bayar..." id="prd_name" name="search" type="hidden">
    ...
    <table width="1088" class="table table-striped table-bordered table-condensed" id="example1">
        ...
        if ((isset($_POST['submit'])) AND ($_POST['search'] <> "da")) {
            $search = $_POST['search'];
            $sql1 = mysqli_query($koneksidb, "SELECT * FROM siswa WHERE kelas LIKE '%$search%' ORDER BY no_induk ASC LIMIT $mulai, $bariss");
        }

        $nomor = $mulai;
        while ($myData = mysqli_fetch_array($sql1)) {
            $nomor++;
        ...
        ?>
            <tr>
                ...
                <td width="6%">
                    <div align="center"><a href="?page=editsiswa&no_induk=<?php echo $myData['no_induk']; ?>">Ubah</a></div>
                </td>
                <td width="48" align="center"> <a href="?page=hapus_siswa2&no_induk=<?php echo $myData['no_induk']; ?>">Hapus</a></td>
            </tr>
        <?php } ?>
        <tr>
            <td colspan="10"><strong>Jumlah Data :</strong> <?php echo $jmlData; ?> </td>
        </tr>
    </table>
    <table width="100%" border="0">
        <tr>
            <td><strong>Jumlah Semua Data Siswa : <?php echo $jmlData; ?></strong></td>
            <td align="right"><strong> <?php "}" ?></strong></td>
        </tr>
    </table>
</div>
