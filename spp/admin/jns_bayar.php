...
<div class="main-content">
    ...
    <table width="1068" class="table table-striped table-bordered table-condensed" id="example1">
        ...
        if (isset($_POST['submit']) && !empty($_POST['search'])) {
            $search = $koneksidb->real_escape_string($_POST['search']);
            $sql1 = $koneksidb->query("SELECT * FROM jns_bayar WHERE jns_bayar LIKE '%$search%'") or die($koneksidb->error);
        }
        ...
        ?>
            <tr>
                ...
                <td width="75" align="center"><a href="?page=editjenis&kode_jenis=<?php echo $data['kode_jenis']; ?>" target="_self" alt="Edit Data"><i class="fa fa-edit"></i> Ubah</a></td>
        ...
        <tr>
            ...
            <td colspan="2" align="right"><strong>Halaman ke :</strong>
                <?php for ($h = 1; $h <= $max; $h++) {
                    $list[$h] = $row * $h - $row;
                    echo "<a href='?page=jns_bayar&hal=$list[$h]'>$h</a> ";
                } ?>
            </td>
        </tr>
    </table>
